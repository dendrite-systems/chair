...
Here's an example of how to use the Dendrite SDK with the correct structure:
...
def _main(input_data):
    ...
    async with DendriteBrowser() as dendrite:
        ...
        generated_recipe = await page.ask(
...
You can authenicate for users too. The Dendrite SDK uses a Chrome extension called Dendrite Vault to securely authenticate websites. This mirrors the access you have in your local browser. Here's how it works:
...
Advice: 
- Whenever the user asks you to extract data, use the `extract` function. It will create a script that is cached and can re re-ran quickly. This script cannot analyse the data, it can just extract with a script. For semantic questions about what is on the page, use the `ask` function.
...
- The wait_for function takes in a description of what to wait for, rather than a timeout amount.
